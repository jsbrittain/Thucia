...
- Tests: `uv run pytest` (CI uses `uv run pytest --cov=thucia --cov-report=term --cov-report=html`). Heavy real-model fits are excluded by default (`addopts = "-m 'not slow'"`); run them explicitly with `uv run pytest -m slow`. Run `uv run pytest tests/thucia/core/models/test_wis.py::test_perfect_forecast_zero_wis` for a single test.
...
- **`pipeline/analysis_core.py` / `analysis_flow.py` are use-case examples, not library code** and are scheduled for deletion. `analysis_core.py` is a 4,400-line monolith of `Steps` toggle blocks (computation + research/plotting); don't extend it, and prefer `thucia.core.pipeline` for new work.
...
- Unit tests under `tests/thucia/core/...` (cases, fs, models, geo, cache). `tests/thucia/core/pipeline/test_pipeline_stages.py` probes each `thucia.core.pipeline` stage in isolation (mocked GADM, small data, fast baseline fit); `tests/thucia/core/pipeline/test_pipeline_e2e.py` chains all stages end-to-end on small synthetic data (raw line-list → padded cases → mocked covariate merge → model inputs → baseline fit → scoring, plus a DuckDB round-trip). `tests/thucia/pipeline/` uses generated data (`generate_test_data.py` writes `tests/thucia/pipeline/test_data/`); its `test_merge_covariates` and `test_model_fitting` are marked `@pytest.mark.skip` (superseded by the stage and e2e tests).
...
- Coverage is ~61% incl. the forecast tests (`darts/__init__`, `sarima`, `tcn`, `nhits`, `tft`, `xgboost`, `movavg`, `baseline`, the `core/pipeline` stages, `registry`, and the geo plugin loader are 79-100%). The remaining gaps (`viz/`, `cli/steps.py`, `geo/stats.py`, deep DartsBase `_extract_horizons` — dead code) are documented; `nbeats`/`tide` bodies are covered only by the `slow` marker (or, for tide's fit, not at all due to instability).
- `tests/thucia/core/containers/test_containers.py` builds/runs an alpine image and fails if no container runtime is running — skip it locally if Docker/Podman isn't up.
