...
- Coverage is ~57% incl. the forecast tests (`darts/__init__`, `sarima`, `tcn`, `nhits`, `tft`, `xgboost`, `movavg`, `baseline` are 79-100%). The remaining gaps (`viz/`, `flow/`, `cli/steps.py`, `geo/stats.py`, `core/models/utils/covariates.py`, deep DartsBase `_extract_horizons` — dead code) are documented; `nbeats`/`tide` bodies are covered only by the `slow` marker (or, for tide's fit, not at all due to instability).
- `tests/thucia/core/containers/test_containers.py` builds/runs an alpine image and fails if no container runtime is running — skip it locally if Docker/Podman isn't up.
