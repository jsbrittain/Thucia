...
- Unit tests under `tests/thucia/core/...` (cases, fs, models, geo, cache). `tests/thucia/core/pipeline/test_pipeline_stages.py` probes each `thucia.core.pipeline` stage (mocked GADM, small data, fast baseline fit); `tests/thucia/pipeline/` uses generated data (`generate_test_data.py` writes `tests/thucia/pipeline/test_data/`); its `test_merge_covariates` and `test_model_fitting` are marked `@pytest.mark.skip` (superseded by the stage tests).
